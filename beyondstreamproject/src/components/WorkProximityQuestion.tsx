...
interface WorkProximityQuestionProps {
  onNext: () => void;
}
...
export function WorkProximityQuestion({ onNext }: WorkProximityQuestionProps) {
  const [selectedOptions, setSelectedOptions] = useState<Set<string>>(new Set());
  const { addMultipleSignals, setWorkProximity, completeStep } = useExplorationStore();

  const options: OptionType[] = [
    { 
      id: 'people', 
      label: 'Working with people or communities', 
      signals: { social: 2 }
    },
    { 
      id: 'creating', 
      label: 'Creating or building things', 
      signals: { creative: 2 }
    },
    { 
      id: 'analyzing', 
      label: 'Analyzing data, logic, or systems', 
      signals: { analytical: 2 }
    },
    { 
      id: 'managing', 
      label: 'Managing processes or operations', 
      signals: { structured: 2 }
    },
    { 
      id: 'teaching', 
      label: 'Teaching, mentoring, or explaining', 
      signals: { social: 1, analytical: 1 }
    },
    { 
      id: 'selling', 
      label: 'Selling, influencing, or persuading', 
      signals: { social: 1, exploratory: 1 }
    },
  ];
  ...
  const handleContinue = () => {
    ...
    const workProximity: string[] = [];
    
    selectedOptions.forEach(optionId => {
      const option = options.find(opt => opt.id === optionId);
      if (option) {
        workProximity.push(option.id);
    ...
    completeStep('work-proximity-question');
    
    // Proceed to next screen
    onNext();
  };

  const progress = 15; // First question in pivot flow, showing 15% progress
    ...
    <div className="min-h-[calc(100vh-96px)] w-full flex flex-col px-8 py-12">
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="max-w-3xl mx-auto w-full space-y-10">
          ...
          >
            What kind of work have you been closest to recently?
          </motion.h2>
              ...
              return (
                <motion.button
                  ...
                  style={{
                    ...
                    border: isSelected ? `3px solid ${color.border}` : `2px solid ${color.border}50`,
                    cursor: 'pointer',
                    textAlign: 'left',
